...
import {
  ...
  fetchAllPublications, isTimestampWeekAgo, isTimestampDayAgo, writeStatus
...
app.get('/pub-ind-pro', async (req, res) => {
  ...
  if(!rawPublicationDownloadStatus.success) {
     if(!rawPublicationDownloadStatus.timestamp) {
    // No status.json file.
    console.log('No status file')
      messages.push("Latest publication data download status from OpenAlex was not found!!");
      writeStatus(false, true, currentEnvHash,  Number(process.env.SEMANTIC_PUBS_START_DATE.split('-')[0]), messages);
      updateBasePublications(currentEnvHash);
    } else {
      if (rawPublicationDownloadStatus.downloadInProgress?.status && !isTimestampDayAgo(rawPublicationDownloadStatus.downloadInProgress?.timestamp)) {
      // Download is in progress.  
      } else {
        messages.push("Latest publication data download status from OpenAlex was not found!!");
        updateBasePublications(currentEnvHash);
        await writeStatus(false, true, currentEnvHash,  Number(process.env.SEMANTIC_PUBS_START_DATE.split('-')[0]), messages);
      }
    }  
    ...
    const resultWithMeta = {
      ...
      data: [],
      message: messages,
      publicationsSince:  Number(process.env.SEMANTIC_PUBS_START_DATE.split('-')[0])
    }
    console.log('rturning');
    res.status(200).send(resultWithMeta);
    return
  }
  ...
  const downloadInProgressOld = downloadInProgress.status ? isTimestampDayAgo(downloadInProgress.timestamp) : true
  let initiateFreshPublicationDownload;
  if (success && !isTimestampWeekAgo(timestamp)) {
    initiateFreshPublicationDownload = downloadInProgressOld && envFileChange;
    console.log("Publications data recently updated.");
    if(envFileChange) {
        messages.push("You are viewing the most recent publication data. However some data sources have been changed.")
    }
  } else {
    if (success) {
    ...
    initiateFreshPublicationDownload = downloadInProgressOld && true;
  }
  console.log(`success: ${success}; initiateFreshPublicationDownload: ${initiateFreshPublicationDownload}; downloadInProgress: ${downloadInProgress}`)
  ...
  try {
    ...
    if (initiateFreshPublicationDownload) {
      writeStatus(success, !downloadInProgressOld, currentEnvHash, publicationsSince, messages)
      updateBasePublications(currentEnvHash);
    }
    res.status(200).send(resultWithMeta);
  } catch (err) {
