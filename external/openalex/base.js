...
import { backupAndWriteFile, createDirIfNotExists } from "../../utils/file-utils.js";
...
class SimplifiedPubs {
  constructor() {
    ...
    createDirIfNotExists('.data/pubs');
  }
...
export const isTimestampDayAgo = (timestamp) => {
  const givenDate = new Date(timestamp);
...
export const writeStatus = (success, downloadInProgress, downloadSettingsHash, publicationsSince, message, timestamp) => {
  const statusJSON = JSON.stringify({
    success,
    downloadInProgress: {status: downloadInProgress, timestamp: new Date().toISOString()},
